...
from classes import String, Number, Output
...
def convert(data, variable, timestep):
    ...
    contour_data = {
        ...
        'values': variable[timestep][:lat_select_index, :lon_select_index]
        .reshape(variable[timestep][:lat_select_index, :lon_select_index].size)
        .tolist()
    }
...
def toNetCDFDataset(source, variable, data):
    ...
    def _copy_variable(target_file, source_file, variable):
        ...
        target_var = target_file.createVariable(variable,
                                                src_var.datatype,
                                                src_var.dimensions)
...
client = GirderClient(String(host, format="json"),
                      Number(port, format="json"))
client.token = String(token, format="json")
...
if not os.path.exists(os.path.join(output_dir, input_file_name)):
    with timer("Downloading %s to %s" %
               (fileId, os.path.join(output_dir, input_file_name))):
...
with timer("Finished running netcdf_mean"):
    data = netcdf_mean(os.path.join(output_dir, input_file_name),
                       String(variable, format="json"),
                       grid_chunk_size,
                       partitions)

with timer("Finished converting to contour JSON"):
    contour = convert(data, String(variable, format="json"), 0)
...
output_item_id = Output(output_item['_id'], type="string", format="json")
